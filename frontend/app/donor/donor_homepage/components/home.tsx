  ...
  useEffect(() => {
    if (email) {
      const fetchStats = async () => {
        try {
          const response = await axios.get(`http://localhost:9500/donor/donorStats/${email}`);
    ...
    <div className="p-6">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        <Card className="p-6 border rounded-lg">
          <CardHeader>
            <CardTitle className="font-medium mb-2">Quick Stats</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex flex-col space-y-2">
              <div className="flex items-center justify-between">
                <span>Donations Count:</span>
                <Badge variant="outline">{stats.donationsCount}</Badge>
              </div>
              <div className="flex items-center justify-between">
                <span>Rating:</span>
        ...
        <Card className="p-6 border rounded-lg">
          <CardHeader>
            <CardTitle className="font-medium mb-2">Recent Activity</CardTitle>
          </CardHeader>
          <CardContent>
            {/* Add content */}
          </CardContent>
        ...
        <Card className="p-6 border rounded-lg">
          <CardHeader>
            <CardTitle className="font-medium mb-2">Notifications</CardTitle>
          </CardHeader>
          <CardContent>
            {/* Add content */}
          </CardContent>
